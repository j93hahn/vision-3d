...
from render import (
    compute_intrinsics, compute_extrinsics, rays_through_pixels,
)
...
def generate_spiral_camera_trajectory(mesh, num_steps=20, num_rounds=1):
    verts = np.asarray(mesh.vertices)
    bbox = compute_axis_aligned_bbox(verts)
    ...
    for eye in eyes:
        ...
        E = compute_extrinsics(eye, center - eye, [0, 1, 0])
        Es.append(E)
    return Es
...
def draw_camera(K, E, img_w, img_h, scale=0.3):
    ...
    # pts is in camera space; transform it to world space
    pts = inv(E) @ pts.T
    pts = pts.T
